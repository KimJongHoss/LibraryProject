    ...
    <body id="page-top">
		...
		<!-- 페이징시 -->
			   <div style="display: flex; justify-content: center;">
				   <c:if test="${prev}">
				      <span>[ <a href="${contextPath }/free/listPage?page=${startNum - 1}">이전</a> ] </span>
				   </c:if>

				   <c:forEach begin="${startNum}" end="${endNum}" var="num">
				       <span style="margin: 0px 10px 0px 0px;">
				         <a href="${contextPath }/free/list?page=${num}">${num}</a> 
				      </span>
				   </c:forEach>
				
				   <c:if test="${next}">
				      <span>[ <a href="${contextPath }/free/list?page=${endNum + 1}">다음</a> ] </span>
				   </c:if>
				  </div>
		<!-- 페이징 끝 -->
